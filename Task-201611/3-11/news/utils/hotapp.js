...
function setDebug(a) { hotAppDebug = void 0 === a ? !1 : a }

function log(a) { hotAppDebug && console.log(a) }

function getOpenID() {
    // console.log(hotAppOpenIdCache);
    return wx.getStorageSync(hotAppOpenIdCache) 
}

function setOpenID(a) { wx.setStorageSync(hotAppOpenIdCache, a) }

function getLocalKey() {
    var a = wx.getStorageSync("key");
    return a ? a : parseInt(9E4 * Math.random() + 1E4, 10) }
...
function login(a) {
    ...
    wx.login({
        success: function(b) {
            if (b.code) wx.request({ url: hotAppHost + "/data/wechat/login", data: { hotAppKey: getHotAppKey(), code: b.code }, method: "POST", success: function(b) {
                    return (b = b.data.openid) ? (setOpenID(b), "function" == typeof a && a(b)) : "function" == typeof a && a(getFakeOpenID()) }, fail: function() {
        ...
        success: function(a) {
            getOpenID() ? wx.getUserInfo({ 
                    success: function(a) { 
                        ...
                        data: { hotAppKey: getHotAppKey(), code: a.code },
                        method: "POST",
                        success: function(a) {
                            console.log(a);
                        ...
                        fail: function() { wx.getUserInfo({ success: function(a) { userInfo = a.userInfo;
                                    sendLaunch() } }) }
                    }) : wx.getUserInfo({ 
                        success: function(a) { 
                            userInfo = a.userInfo;
...
function sendLaunch() {
    wx.request({
        ...
        data: { hotAppKey: getHotAppKey(), openId: getOpenID(), hotAppUUID: getHotAppUUID(), userInfo: getUserInfo(), systemInfo: getSystemInfo(), phoneTime: Date.parse(new Date) / 1E3, hotAppVersion: hotAppVersion },
        method: "POST",
        success: function(a) {
            ...
            if (0 != uploadType) {
                ...
                0 != f.length && wx.request({
                    ...
                    success: function(a) { log(wx.getStorageSync("hotAppEvent") || []);
                        log(a.data);
                        try { wx.removeStorageSync("hotAppEvent") } catch (c) { log(c) } },
                    fail: function() { log("send event fail");
                        wx.setStorageSync("hotAppEvent", f) }
                })
            }
        },
        fail: function(a) { log("send launch fail " + a) }
    })
}
...
function onEvent(a, f) {
    ...
    if ("" == hotAppKey) log("hotappkey is empty");
        ...
        var b = wx.getStorageSync("hotAppEvent") || [],
            c = { eventId: a, eventValue: f, phoneTime: Date.parse(new Date) / 1E3 };
        b.push(c);
        0 != uploadType ? wx.setStorageSync("hotAppEvent", b) : wx.request({
            url: hotAppHost + "/data/wechat/event",
            data: { hotAppKey: getHotAppKey(), openId: getOpenID(), hotAppUUID: getHotAppUUID(), eventArray: b, hotAppVersion: hotAppVersion },
            method: "POST",
            success: function(a) { log(wx.getStorageSync("hotAppEvent") || []);
...
function setEventUploadType(a) { uploadType = a }
var hexcase = 0,
    b64pad = "",
    chrsz = 8;
...
function get(a, f) { http(hotAppHost + "/api/get", { appkey: hotAppKey, key: a }, f) }

function post(a, f, b) { http(hotAppHost + "/api/post", { appkey: hotAppKey, key: a, value: f }, b) }

function del(a, f) {
    var b = hotAppHost + "/api/delete",
        c = { appkey: getHotAppKey(), key: a };
    http(b, c, f) }

function http(a, f, b) { wx.request({ url: a, data: f, method: "post", header: { "content-type": "application/json" }, success: function(a) {
            return "function" == typeof b && b(a.data) }, fail: function() {
            return "function" == typeof b && b(!1) } }) }

function request(a) { 0 == a.useProxy ? wx.request({ url: a.url, data: a.data, header: a.header, method: a.method, success: function(f) { a.success(f) }, fail: function(f) { a.fail(f) }, complete: function(f) { a.complete(f) } }) : "" == hotAppKey ? log("hotappkey is empty") : wx.request({ url: hotAppHost + "/proxy/?appkey=" + hotAppKey + "&url=" + a.url, data: a.data, header: a.header, method: a.method, success: function(f) { a.success(f) }, fail: function(f) { a.fail(f) }, complete: function(f) { a.complete(f) } }) }
export default  { init: init, onEvent: onEvent, setEventUploadType: setEventUploadType, clearData: clearData, wxlogin: login, getFakeOpenID: getFakeOpenID, getOpenID: getOpenID, getPrefix: getPrefix, genPrimaryKey: genPrimaryKey, replaceOpenIdKey: replaceOpenIdKey, searchkey: searchkey, get: get, post: post, del: del, request: request, getVersion: getVersion, setDebug: setDebug };
